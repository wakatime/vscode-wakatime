...
export class Stats {
    ...
    constructor(options: Options, logger: Logger) {
        ...
        this.logger = logger;
        this.options.getSetting('settings', 'api_key', (_, val) => { this.apiKey = val; });
        this.options.getSetting('settings', 'proxy', (_, val) => { this.proxy = val; });
    }
    ...
    public async getCodingActivity(): Promise<string> {
        return new Promise<string>(async (resolve, reject) => {
            ...
            const url = `https://wakatime.com/api/v1/users/current/summaries?api_key=${this.apiKey}&start=${today}&end=${today}`;
            let params = { url: url };
